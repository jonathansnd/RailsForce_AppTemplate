...
class Chatter
  ...
  def set_headers
    Chatter.headers 'Authorization' => "OAuth #{@accesstoken}"
  end

  def root_url
    @root_url = @instanceurl+"/services/data/v"+ENV['sfdc_api_version']+"/chatter"
  end
  ...
  def get_users_info(id)
    ...
    # if(@resp.code == 401)
    #   ForceUtils.refreshToken(@current_user,@service_id)
    ...
    set_headers
     ...
     @feed = Hash.new{}
      @feed["feedItems"] = Hash.new{} 
       ...
       @feed["feedItems"] = Chatter.get(@instanceurl+refpath)
       return @feed
  end
